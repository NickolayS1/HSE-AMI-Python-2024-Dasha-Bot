...
class DataBase:
    ...
    async def get_type_of_group(self, group_id: int) -> DataBaseResponses:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT group_type FROM Groups WHERE group_id=?", (group_id,))
    ...
    async def set_type_of_group(self, group_id: int, group_type: GroupTypes) -> DataBaseResponses:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("INSERT OR REPLACE INTO Groups (group_id, group_type) VALUES (?, ?)", (group_id, group_type))
            conn.commit()
            return DataBaseResponses.SUCCESS
    ...
    async def get_user_type(self, user_id: int, group_id: int) -> UserTypes:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT user_type FROM Users WHERE user_id=? AND group_id=?", (user_id, group_id))
            result = cursor.fetchone()
    ...
    async def set_user_type(self, user_id: int, group_id: int, new_type: UserTypes) -> DataBaseResponses:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("INSERT OR REPLACE INTO Users (user_id, group_id, user_type) VALUES (?, ?, ?)",
                           (user_id, group_id, new_type))
            conn.commit()
            return DataBaseResponses.SUCCESS
    ...
    async def get_amount_of_maximum_warns(self, group_id: int) -> int:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT max_group_warns FROM Groups WHERE group_id=?", (group_id,))
            result = cursor.fetchone()
    ...
    async def set_amount_of_maximum_warns(self, group_id: int, max_warns: int) -> DataBaseResponses:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("INSERT OR REPLACE INTO Groups (group_id, max_group_warns) VALUES (?, ?)",
                           (group_id, max_warns))
            conn.commit()
            return DataBaseResponses.SUCCESS
    ...
    async def get_amounts_of_warns(self, user_id: int, group_id: int) -> int:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT user_warns FROM Users WHERE user_id=? AND group_id=?",
                           (user_id, group_id))
            result = cursor.fetchone()
            return result[0] if result else None
